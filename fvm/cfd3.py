...
if __name__ == '__main__':
    ...
    rho_u = Variable2d(mesh, np.zeros(len(mesh.nodes)), boundary_none, "rho_u")
    rho_v = Variable2d(mesh, np.zeros(len(mesh.nodes)), boundary_none, "rho_v")
    p_e = Variable2d(mesh, np.zeros(len(mesh.nodes)), boundary_none, "p_e")
    dt_var = DT(update_fn=DT.UpdatePolicies.CourantFriedrichsLewy, CFL=CFL, space_vars=[u, v])

    equation_system = [
        [d1t(rho), "=", -d1dx(rho_u) - d1dy(rho_v)],
