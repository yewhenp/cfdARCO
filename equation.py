...
class Variable2d(Variable):
    ...
    def evaluate_second_order(self, **kwargs):
        ...
        if kwargs["direction"] == "y":
            # arr_value[:, 1:-1] = (arr_value[:, 2:] - 2*arr_value[:, 1:-1] + arr_value[:, :-2]) / (self.deltas[1] ** 2)
...
class _D2T(Variable):
    ...
    def extract(self, left_part, **kwargs):
        if len(self.var.history) == 0:
            return (kwargs["dt"] * left_part) + self.var.current
        return ((kwargs["dt"]**2) * left_part) - self.var.history[-1] + 2*self.var.current
        ...
        for t in tqdm.trange(self.timesteps):
            for i in range(len(equations)):
                ...
                for _ in range(qq):
                    ...
                    all_vars[i].set_current(extracted)
